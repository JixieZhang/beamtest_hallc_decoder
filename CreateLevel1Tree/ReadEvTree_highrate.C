...
//double gPed7[16]={108.1, 104.7, 100.6, 101.8, 101.5,  98.6, 103.1,  96.2, 108.4, 102.7, 107.6, 111.3, 105.2, 103.0, 104.4,  99.9};  //20230112
//double gPed8[16]={117.0, 109.7, 121.1, 116.4, 103.9, 104.0, 107.5, 111.0, 110.5, 105.6, 105.2, 102.5, 106.0, 107.9, 113.1, 103.2};  //20230112

//20230120-20230123 data set:  4023 <= runs <=4066 
//double gPed7[16]={106.7, 104.0,  99.3, 101.7, 101.3,  98.7, 103.8,  95.1, 109.3, 102.7, 107.5, 110.6, 105.4, 101.8, 103.5,  99.4};  //20230120
//double gPed8[16]={115.1, 110.7, 122.8, 118.7, 105.1, 105.0, 108.5, 107.2, 107.0, 102.1, 104.4, 100.7, 104.9, 107.7, 113.3, 103.3};  //20230120

//run 4085, 20230127, 4067<=runs<=4101  
double gPed7[16]={107.0, 105.0, 100.1, 102.3, 101.7,  99.3, 103.9,  96.4, 110.1, 103.5, 108.5, 110.4, 105.6, 102.5, 104.2,  99.6};  //20230127
double gPed8[16]={118.1, 110.0, 126.7, 128.3, 105.3, 105.6, 108.4, 108.3, 108.0, 105.8, 101.6, 104.4, 105.2, 108.3, 115.4, 104.1};  //20230127
/*
//run 4112 20230201
double gPed7[16]={105.9, 101.6,  99.6, 100.4,  99.8,  98.1, 101.4,  94.5, 107.1, 100.3, 106.9, 108.0, 103.3, 100.9, 101.4,  98.7};  //20230201
double gPed7[16]={107.4, 107.9, 105.1, 116.7, 103.0, 104.0, 106.8, 106.7, 106.2, 101.8, 100.0, 102.5, 103.3, 107.1, 108.3, 101.9};  //20230201
*/
////////////////////////////////////////////////////////////////////////
/*
 * //run 4061:
  const float kSlot7PeakPos_2[] = ;
  const float kSlot8PeakPos_2[] = ;
//run 4063:
  const float kSlot7PeakPos_1[] = ;
  const float kSlot8PeakPos_1[] = ;
//run 4058:
  const float kSlot7PeakPos_3[] = ;
  const float kSlot8PeakPos_3[] = ;
/run 4055:
  const float kSlot7PeakPos_4[] = { 86.5, 53.5, 47.5, 41.5,  1.5, 32.5,  1.5,  0.5, 69.5, 10.5, 17.5,  9.5, 45.5, 47.5, 44.5, 11.5 };
  const float kSlot8PeakPos_4[] = { 78.5,  4.5, 43.5, 80.5, 38.5, 37.5,  1.5, 84.5, 84.5, 84.5, 35.5, 36.5, 36.5, 83.5, 79.5, 98.5 };

// peak position for trig type 1-6, for runs 4023-4066 only (20230120-20230123 data set), bit 4,5,6 are not available
const double kSlot7PeakPos[][16] = {
...
const double kSlot8PeakPos[6][16] = { 
  { 28.5, 23.5, 78.5, 26.5, 25.5, 25.5, 19.5, 26.5, 22.5, 23.5, 25.5, 25.5, 26.5, 27.5, 28.5, 77.5 },
  { 24.5, 18.5, 24.5, 21.5, 21.5, 21.5, 14.5, 18.5, 18.5, 19.5, 18.5, 24.5, 25.5, 20.5, 21.5, 98.5 },
  { 24.5, 22.5, 33.5, 26.5, 22.5, 21.5, 19.5, 25.5, 24.5, 25.5, 24.5, 25.5, 26.5, 27.5, 27.5, 77.5 },
  { 31.5, 26.5, 34.5, 29.5, 29.5, 28.5, 22.5, 26.5, 25.5, 26.5, 26.5, 27.5, 28.5, 30.5, 28.5, 98.5 },
  { 31.5, 26.5, 34.5, 29.5, 29.5, 28.5, 22.5, 26.5, 25.5, 26.5, 26.5, 27.5, 28.5, 30.5, 28.5, 98.5 },
  { 31.5, 26.5, 34.5, 29.5, 29.5, 28.5, 22.5, 26.5, 25.5, 26.5, 26.5, 27.5, 28.5, 30.5, 28.5, 98.5 }
};
*/
// peak position for trig type 1-6, for runs 4084-4101 only (20230127 data set), bit 3,5,6 are not available
const double kSlot7PeakPos[][16] = {
...
const double kSlot8PeakPos[6][16] = { 
  { 40.5, 32.5, 40.5, 35.5, 35.5, 34.5, 28.5, 35.5, 31.5, 33.5, 34.5, 33.5, 34.5, 34.5, 37.5, 98.5 },
  { 33.5, 28.5, 33.5, 31.5, 31.5, 30.5, 24.5, 28.5, 27.5, 28.5, 28.5, 26.5, 27.5, 29.5, 31.5, 98.5 },
  { 33.5, 28.5, 33.5, 31.5, 31.5, 30.5, 24.5, 28.5, 27.5, 28.5, 28.5, 26.5, 27.5, 29.5, 31.5, 98.5 },
  { 31.5, 26.5, 34.5, 29.5, 29.5, 28.5, 22.5, 26.5, 25.5, 26.5, 26.5, 27.5, 28.5, 30.5, 28.5, 98.5 },
  { 31.5, 26.5, 34.5, 29.5, 29.5, 28.5, 22.5, 26.5, 25.5, 26.5, 26.5, 27.5, 28.5, 30.5, 28.5, 98.5 },
  { 31.5, 26.5, 34.5, 29.5, 29.5, 28.5, 22.5, 26.5, 25.5, 26.5, 26.5, 27.5, 28.5, 30.5, 28.5, 98.5 }
...
void ReadEvTree::Loop(Long64_t istart, Long64_t iend)
{
  ...
  for (Long64_t jentry=istart; jentry<iend;jentry++) {
    ...
    for(int ib=5;ib>=0;ib--) {
      int ret = (trigger_type & int(pow(2.0,ib)));
      if(ret) {trigbit=ib+1;break;} 
    } 
    if(!trigbit) continue;
    ...
    for (int ich=0; ich<16; ich++) {
      //for()  //TODO change to loop over the whole vector
      ...
      if (fabs(pos-kSlot7PeakPos[trigbit-1][ich])>5.0) continue;
      ...
      float adcSum = 0;
      int left=(int)(Slot7Data[ich]->peaks[0].left);
      int right=(int)(Slot7Data[ich]->peaks[0].right);
    ...
    for (int ich=0; ich<15; ich++) {
      ...
      //the next line is to cut out-of-time pulse, make this cut wide if the peak location is not known yet
      if (fabs(pos-kSlot8PeakPos[trigbit-1][ich])>5.0) continue;
      ...
      float adcSum = 0;
      int left=(int)(Slot8Data[ich]->peaks[0].left);
      int right=(int)(Slot8Data[ich]->peaks[0].right);
      for (int iw = left; iw <= right; iw++) {
...
void DoPedestal(int run_start, int run_end)
{
  ...
  for (int run=run_start;run<=run_end;run++) {
    //sprintf(filename,"../ROOTFILE/beamtest_hallc_%4d_0.root",run);  //4110-4113
    //sprintf(filename,"../ROOTFILE/highrate_20230124/beamtest_hallc_%4d_0.root",run);  //4067-4069
    //sprintf(filename,"../ROOTFILE/highrate_20230127/beamtest_hallc_%4d_0.root",run);  //4084-4086
    sprintf(filename,"../ROOTFILE/highrate_BeamOffHvOn/beamtest_hallc_%4d_0.root",run);  //4023-4036
...
void DoLevel1Tree(int run_start, int run_end)
{
  ...
  for (int run=run_start;run<=run_end;run++) {
    ...
    for (int subrun=0;subrun<500;subrun++) {
      sprintf(filename,"../ROOTFILE/highrate_BeamOffHvOn/beamtest_hallc_%04d_%d.root",run,subrun);
