...
namespace Decoder
{
    ...
    THaSlotData::THaSlotData() : crate(-1), slot(-1), fModule(nullptr), numhitperchan(0), numraw(0), numchanhit(0),
                                 firstfreedataidx(0), numholesdataidx(0), fDebugFile(nullptr),
                                 didini(false), fNchan(0) {}

    //_____________________________________________________________________________
    THaSlotData::THaSlotData(UInt_t cra, UInt_t slo) : crate(cra), slot(slo), fModule(nullptr), numhitperchan(0), numraw(0), numchanhit(0),
                                                       firstfreedataidx(0), numholesdataidx(0), fDebugFile(nullptr),
                                                       didini(false), fNchan(0)
    {
    }
    ...
    int THaSlotData::loadModule(const THaCrateMap *map)
    {
        ...
        if (found != Module::fgModuleTypes().end())
        {
            ...
            if (fDebugFile)
                *fDebugFile << "THaSlotData::  Found Module !!!! " << dec << modelnum << endl;

            if (fDebugFile)
                *fDebugFile << "THaSlotData:: Creating fModule" << endl;
    ...
    UInt_t THaSlotData::LoadIfSlot(const UInt_t *evbuffer, const UInt_t *pstop)
    {
        ...
        {
            if (fDebugFile)
                *fDebugFile << "THaSlotData:: Not slot ... return ... " << endl;
            return 0;
        }
        if (fDebugFile)
            fModule->DoPrint();
    ...
    UInt_t THaSlotData::LoadBank(const UInt_t *p, UInt_t pos, UInt_t len)
    {
        ...
        if (fDebugFile)
            *fDebugFile << "THaSlotData:: after LoadBank:  wordseen =  " << dec << "  " << wordseen << endl;
        return wordseen;
    }
    ...
    void THaSlotData::print_to_file() const
    {
        ...
        *fDebugFile << "Total Amount of Data : " << dec << getNumRaw() << endl;
        ...
        {
            for (UInt_t j = 0; j < 5; j++)
                *fDebugFile << getRawData(k++) << "  ";
            *fDebugFile << endl;
        }
        for (UInt_t i = k; i < getNumRaw(); i++)
            *fDebugFile << getRawData(i) << "  ";
        bool first = true;
        for (UInt_t chan = 0; chan < fNchan; chan++)
        {
            if (getNumHits(chan) > 0)
