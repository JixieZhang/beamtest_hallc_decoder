...
public:
    ...
    {
        assert(ScalersEnabled() && fgAllowUnimpl);
        return kMaxUInt;
    };
...
protected:
    ...
    class RocDat_t
    { // Coordinates of ROC data in raw event
    public:
...
inline Bool_t THaEvData::InCrate(UInt_t crate, UInt_t i) const
{
    ...
    if (crate == 0)
        return (i < GetEvLength());
    if (rocdat[crate].pos == 0 || rocdat[crate].len == 0)
        return false;
...
inline Bool_t THaEvData::IsPhysicsTrigger() const
{
    return ((event_type > 0) && (event_type <= Decoder::MAX_PHYS_EVTYPE));
}

inline Bool_t THaEvData::IsScalerEvent() const
{
...
inline Double_t THaEvData::GetEpicsData(const char * /*tag*/, UInt_t /*event*/) const
{
    assert(IsLoadedEpics("") && fgAllowUnimpl);
    return kBig;
}

inline Double_t THaEvData::GetEpicsTime(const char * /*tag*/, UInt_t /*event*/) const
{
    assert(IsLoadedEpics("") && fgAllowUnimpl);
    return kBig;
}

inline TString THaEvData::GetEpicsString(const char * /*tag*/, UInt_t /*event*/) const
{
    assert(IsLoadedEpics("") && fgAllowUnimpl);
    return {""};
}

inline Bool_t THaEvData::HasCapability(Decoder::EModuleType type, UInt_t crate,
                                       UInt_t slot) const
{
...
inline Bool_t THaEvData::IsMultifunction(UInt_t crate, UInt_t slot) const
{
    Decoder::Module *module = GetModule(crate, slot);
    if (!module)
    {
...
inline UInt_t THaEvData::GetNumEvents(Decoder::EModuleType type, UInt_t crate, UInt_t slot, UInt_t chan) const
{
    Decoder::Module *module = GetModule(crate, slot);
    if (!module)
        return 0;
...
inline UInt_t THaEvData::GetData(Decoder::EModuleType type, UInt_t crate, UInt_t slot, UInt_t chan, UInt_t hit) const
{
    Decoder::Module *module = GetModule(crate, slot);
    if (!module)
        return 0;
    if (module->HasCapability(type))
    {
        if (hit >= module->GetNumEvents(type, chan))
            return 0;
...
inline UInt_t THaEvData::GetLEbit(UInt_t crate, UInt_t slot, UInt_t chan, UInt_t hit) const
{
    // get the Leading Edge bit (works for fastbus)
    return GetOpt(crate, slot, chan, hit);
}

inline UInt_t THaEvData::GetOpt(UInt_t crate, UInt_t slot, UInt_t /*chan*/, UInt_t hit) const
{
