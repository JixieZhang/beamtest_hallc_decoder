...
HCTracking::HCTracking()
{
    // Set default values of important parameters:
    Clear();
...
// loop over all possible combinations of layers:
// This only needs to be done ONCE (after loading the geometry from the database!)
void HCTracking::InitLayerCombos()
{
    fLayerCombinations.clear();

    for (int icombo = 0; icombo < pow(2, fNlayers); icombo++)
    {
        std::vector<int> layercombo;
        ...
        if (nlayersoncombo >= fMinHitsOnTrack)
        {
            fLayerCombinations[nlayersoncombo].push_back(layercombo);
        }
    }
}

void HCTracking::InitGridBins()
{
    ...
    for (int ilayer = 0; ilayer < fNlayers; ilayer++)
    {
        ...
        for (auto imod = modlist_layer.begin(); imod != modlist_layer.end(); ++imod)
        {
            ...
            GEMModule *mtemp = fModules[module];
...
void HCTracking::Decode(const EventWrapper &event)
{
    Clear();
...
void HCTracking::InitEfficiencyHistos(const char *dname)
{
...
TVector3 HCTracking::GetHitPosGlobal(int module, int clustindex)
{
    // check that module and cluster are in range: these conditions should never evaluate to true, but we want to prevent
    //  seg. faults anyway
    if (module < 0 || module >= (int)fModules.size())
        return TVector3(-1.e12, -1.e12, -1.e12);
    if (clustindex < 0 || clustindex >= (int)fModules[module]->fHits.size())
        return TVector3(-1.e12, -1.e12, -1.e12);
...
void HCTracking::CalcLineOfBestFit(const std::map<int, int> &hitcombo, double &xtrack, double &ytrack, double &xptrack, double &yptrack)
{
...
TVector2 HCTracking::GetUVTrack(int module, TVector3 track_origin, TVector3 track_direction)
{
...
TVector3 HCTracking::TrackIntersect(int module, TVector3 track_origin, TVector3 track_direction, double &sintersect)
{
...
void HCTracking::FitTrack(const std::map<int, int> &hitcombo, double &xtrack, double &ytrack, double &xptrack, double &yptrack, double &chi2ndf, std::vector<double> &uresid, std::vector<double> &vresid)
{
...
bool HCTracking::GetNextCombo(const std::set<int> &layers, std::map<int, int> &hitcounter, std::map<int, int> &hitcombo, bool &firstcombo)
{
    ...
    for (auto layercounter = layers.begin(); layercounter != layers.end(); ++layercounter)
    {
        ...
        { // Note: if this is the first combination we don't increment hitcounter
            if (hitcounter[layer] + 1 < (int)freehitlist_goodxy[layer].size())
            { // more available hits in current layer. increment hit counter
                hitcounter[layer]++;
            }
            else
            {
        ...
        if (nextlayercounter == layers.end())
            comboexists = false; // we reached the last hit in the last layer. stop iteration
    }
...
int HCTracking::GetGridBin(int module, int hitindex)
{
    if (module < 0 || module >= (int)fModules.size())
    {
        return -1;
    }
    if (hitindex < 0 || hitindex >= (int)fModules[module]->fHits.size())
    {
        return -1;
    }
...
bool HCTracking::PassedOpticsConstraint(TVector3 TrackOrigin, TVector3 TrackDirection, bool coarsecheck)
{
    // for now, do nothing
    return true;
}

bool HCTracking::CheckConstraint(double xtr, double ytr, double xptr, double yptr, bool coarsecheck)
{ // later we should really pass an error matrix of the four track parameters to this routine
...
void HCTracking::Clear()
{
    // Clear out any event-specific stuff
...
double HCTracking::InitHitList()
{
    // clear out any old information:
    layers_with_2Dhits.clear();
    layerswithfreehits.clear();

    double ncombos_all_layers = 1;
...
double HCTracking::InitFreeHitList()
{
    ...
    for (auto ilay = layers_with_2Dhits.begin(); ilay != layers_with_2Dhits.end(); ++ilay)
    {
        ...
        // std::cout << "[HCTracking::find_tracks]: layer, nbins_gridxy = " << layer << ", " << nbins_gridxy << std::endl;
        ...
        {
            // std::cout << "[HCTracking::find_tracks]: layer, N2Dhits_layer[layer] = " << layer << ", "
...
void HCTracking::hit_reconstruction()
{
    ...
    {
        GEMModule *mod = fModules[imodule];
        ...
        if (!fUseConstraint)
        { // call find_2D hits for the module without any constraint:
            mod->find_2Dhits();
        }
        else
        {
    ...
    {
        if (fNstripsU_layer[layerindex] + fNstripsV_layer[layerindex] > 0)
            fNlayers_hit++;
        if (fNstripsU_layer[layerindex] > 0)
            fNlayers_hitU++;
        if (fNstripsV_layer[layerindex] > 0)
            fNlayers_hitV++;
...
void HCTracking::find_tracks()
{
    ...
    Clear();
    // std::cout << "[HCTracking::find_tracks]: finished clearing track arrays..." << std::endl;

    fNtracks_found = 0;

    if (!fclustering_done)
    {
        // This shouldn't be called before hit reconstruction, but if it is, then this routine will call the hit reconstruction:
        hit_reconstruction();
    }

    // std::cout << "[HCTracking::find_tracks]: finished hit reconstruction..." << std::endl;
    ...
    { // Then we have enough layers to do tracking:
        ...
        while (nhitsrequired >= fMinHitsOnTrack)
        { // as long as the current minimum hit requirement exceeds the minimum hits to define a track, we look for more tracks with
            // nhitsrequired hits:
            foundtrack = false;
            ...
            double Ncombos_free = InitFreeHitList();

            if (Ncombos_free > fMaxHitCombinations_Total)
            {
                std::cout << "Warning in [HCTracking::find_tracks]: total potential hit combinations = "
            ...
            {
                // check that the number of layers with free hits is at least equal to the current minimum hit requirement:
                ...
                std::map<int, int> besthitcombo;
                double minchi2 = 1.e20; // arbitrary large number initially

                std::vector<double> besttrack(4); // x, y, x', y'
                ...
                std::vector<double> uresidbest, vresidbest;
                ...
                for (unsigned int icombo = 0; icombo < fLayerCombinations[nhitsrequired].size(); icombo++)
                {
                    ...
                    for (int ihit = 0; ihit < nhitsrequired; ihit++)
                    {
                        ...
                        // std::cout << layer << ", ";
                        if (layerswithfreehits.find(layer) != layerswithfreehits.end())
                        { // check that this layer has unused hits:
                    ...
                    for (auto ibin : binswithfreehits_layer[minlayer])
                    {
                        for (auto jbin : binswithfreehits_layer[maxlayer])
                        {
                            ...
                            if (fUseConstraint && !constraint_check)
                                continue;
                            ...
                            if (fUseOpticsConstraint && !optics_check)
                                continue;
                            ...
                            { // this should not happen under sane conditions
                                std::cout << "Warning in [HCTracking::find_tracks]: skipping bin combination of layer " << minlayer << ", bin " << ibin << " with layer " << maxlayer << ", bin " << jbin << " (this should not happen under sane analysis config and/or experiment conditions)" << std::endl;
                                continue;
                            }
                            //   // if ( false ){
                            ...
                            //  if( ncombos_minmax > fMaxHitCombinations ){
                            //    //  if( false ){
                            //    std::cout << "Warning in [HCTracking::find_tracks]: no combination of two layers found with hit combinations less than maximum of "
                            ...
                            for (int ihit = 0; ihit < Nfreehits_binxy_layer[minlayer][ibin]; ihit++)
                            {
                                for (int jhit = 0; jhit < Nfreehits_binxy_layer[maxlayer][jbin]; jhit++)
                                {
                                    ...
                                    if (!goodoptics)
                                        continue; // skip this pair if we don't pass the good optics requirement:
                                    ...
                                    for (auto ilay = otherlayers.begin(); ilay != otherlayers.end(); ++ilay)
                                    {
                                        ...
                                        for (auto imod = fModuleListByLayer[layer].begin(); imod != fModuleListByLayer[layer].end(); ++imod)
                                        {
                                            ...
                                            if (fModules[modtemp]->IsInActiveArea(intercept))
                                            { // test
                                        ...
                                        if (binxdifflo < fGridEdgeToleranceX && binxlo > 0)
                                            binxlo--;
                                        if (binydifflo < fGridEdgeToleranceY && binylo > 0)
                                            binylo--;
                                        ...
                                        if (fGridBinWidthX - binxdiffhi < fGridEdgeToleranceX && binxhi + 1 < fGridNbinsX_layer[layer])
                                            binxhi++;
                                        if (fGridBinWidthY - binydiffhi < fGridEdgeToleranceY && binyhi + 1 < fGridNbinsY_layer[layer])
                                            binyhi++;
                                    ...
                                    if (nextcomboexists)
                                    {
                                        ...
                                        if (ncombos <= fMaxHitCombinations_InnerLayers)
                                        {
                                            while ((nextcomboexists = GetNextCombo(otherlayers, freehitcounter, hitcombo, firstcombo)))
                                            {
                                                ...
                                                std::vector<double> uresidtemp, vresidtemp;
                                        ...
                                        {
                                            std::cout << "Warning in [HCTracking::find_tracks()]... number of hit combos for inner layers = " << ncombos
                ...
                { // then we found at least one "good" candidate track:
                    ...
                    for (auto ilayer = besthitcombo.begin(); ilayer != besthitcombo.end(); ++ilayer)
                    {
                        ...
                        if (fModules[module]->fHits[iclust].highquality)
                            nHighQualityHits++;
                    }
            ...
            if (!foundtrack)
            { // If we didn't find any tracks at the current minimum hit requirement, then reduce the minimum, and see if we can find a good track
...
void HCTracking::fill_good_hit_arrays()
{
    ...
    if (!fEfficiencyInitialized)
        InitEfficiencyHistos("generic_gemtracker"); // this guarantees that the efficiency histograms will exist when we try to fill them:
    ...
    { // loop over tracks
        ...
        { // loop over hits on tracks:
            ...
            for (unsigned int istrip = uclustinfo->istriplo; istrip <= uclustinfo->istriphi; istrip++)
            {
                ...
                double ccor_temp;
                if (ismaxstrip)
                { // for the max. strip, calculate corr. coeff with the cluster-summed ADC samples:
                    ccor_temp = fModules[module]->CorrCoeff(ntimesamples,
                                                            fModules[module]->fADCsamples[hitidx_i],
                                                            uclustinfo->ADCsamples);
                }
                else
                { // for other strips, calculate corr. coeff. with the max strip:
                    ccor_temp = fModules[module]->CorrCoeff(ntimesamples,
                                                            fModules[module]->fADCsamples[hitidx_i],
            ...
            if (fMakeEfficiencyPlots && fNhitsOnTrack[itrack] >= fMinHitsOnTrack && itrack == 0)
            {
                ...
                if (constraint_check)
                {
                    ...
                    if (fModules[module]->fhdidhitx != NULL)
                        fModules[module]->fhdidhitx->Fill(LocalCoord.X());
                    if (fModules[module]->fhdidhity != NULL)
                        fModules[module]->fhdidhity->Fill(LocalCoord.Y());
                    if (fModules[module]->fhdidhitxy != NULL)
                        fModules[module]->fhdidhitxy->Fill(LocalCoord.Y(), LocalCoord.X());
        ...
        for (int ilayer = 0; ilayer < fNlayers; ilayer++)
        {
            ...
            int moduleontrack = -1;
            if (layersontrack.find(ilayer) != layersontrack.end())
            { // reduce bias in efficiency determination by requiring hits in at least three layers OTHER than the one in question if this layer is on the track:
                minhits = 4;
                ...
                if (inactivearea || module == moduleontrack)
                {
                    ...
                    if (fMakeEfficiencyPlots && fNhitsOnTrack[itrack] >= minhits && itrack == 0)
                    {
                        ...
                        if (constraint_check)
                        {
                            ...
                            if (fModules[module]->fhshouldhitx != NULL)
                                fModules[module]->fhshouldhitx->Fill(LocalCoord.X());
                            if (fModules[module]->fhshouldhity != NULL)
                                fModules[module]->fhshouldhity->Fill(LocalCoord.Y());
                            if (fModules[module]->fhshouldhitxy != NULL)
                                fModules[module]->fhshouldhitxy->Fill(LocalCoord.Y(), LocalCoord.X());
                            ...
                            if (fModules[module]->fStrip_BUILD_ALL_SAMPLES[0] && !fModules[module]->fStrip_ENABLE_CM[0] && fModules[module]->fStrip_CM_GOOD[0] && fNegSignalStudy)
                            {
                                ...
                                for (int istrip = 0; istrip < fModules[module]->fNstrips_hit; istrip++)
                                {
                                    ...
                                    if (!fModules[module]->fStripIsNeg[istrip])
                                        continue; // Skip is the strip is not negative
                                    ...
                                    if (fModules[module]->fAxis[istrip] == SBSGEM::kUaxis)
                                    {
                                        ...
                                        {
                                            if (modules_hit[module])
                                                fNstripsU_layer_neg_miss[ilayer]++;
                                            if (!modules_hit[module])
                                                fNstripsU_layer_neg_hit[ilayer]++;
                                        }
                                    }
                                    ...
                                    if (fModules[module]->fAxis[istrip] == SBSGEM::kVaxis)
                                    {
                                        ...
                                        {
                                            if (modules_hit[module])
                                                fNstripsV_layer_neg_miss[ilayer]++;
                                            if (!modules_hit[module])
                                                fNstripsV_layer_neg_hit[ilayer]++;
                                        }
                                    }
                                }
                                ...
                                for (UInt_t iuclust = 0; iuclust < fModules[module]->fNclustU; iuclust++)
                                {
                                    ...
                                    if (!uclust->isneg)
                                        continue; // skip if cluster is not negative
                                ...
                                for (int iuclust = 0; iuclust < fModules[module]->fNclustU; iuclust++)
                                {
                                    ...
                                    for (int ivclust = 0; ivclust < fModules[module]->fNclustV; ivclust++)
                                    {
                                        ...
                                        if (!uclust->isneg && !vclust->isneg)
                                            continue; // At least one of the clusters must be negative
...
void HCTracking::AddNewTrack(const std::map<int, int> &hitcombo, const std::vector<double> &BestTrack, double chi2ndf, const std::vector<double> &uresidbest, const std::vector<double> &vresidbest)
{
    ...
    std::vector<double> eresidu, eresidv;
...
void HCTracking::PurgeHits(int itrack)
{
    for (int ihit = 0; ihit < fNhitsOnTrack[itrack]; ihit++)
    {
        int module = fModListTrack[itrack][ihit];
...
void HCTracking::CompleteInitialization()
{
    fLayers.clear();
    fLayerByIndex.clear();
    ...
    for (int imod = 0; imod < (int)fModules.size(); imod++)
    {
        if (!fModules[imod])
        {
            std::cout << "XB: " << __func__ << " module :" << imod << " is not initialized." << std::endl;
            continue;
        }
    ...
    { // load pedestals from file; NOTE: This OVERRIDES any pedestals found in the database
    ...
    if (!fcmfilename.empty() && !cm_already_loaded)
    { // load CM from file; NOTE: This OVERRIDES any pedestals found in the database
...
void HCTracking::LoadPedestals(const char *fname)
{
    ...
    std::cout << "[HCTracking::LoadPedestals]: fname = " << fname << std::endl;
    ...
    { //
        ...
        std::cout << "[HCTracking::LoadPedestals]: could not find " << fname << " in working directory, looking in " << prefix << std::endl;
    ...
    {
        std::cout << "Found pedestal file " << pedfilename << std::endl;
    }
...
void HCTracking::LoadCM(const char *fname)
{

    std::cout << "[HCTracking::LoadCM]: fname = " << fname << std::endl;
    ...
    { //
        ...
        std::cout << "[HCTracking::LoadCM]: could not find " << fname << " in working directory, looking in " << prefix << std::endl;
    ...
    {
        std::cout << "Found CM file " << cmfilename << std::endl;
    }
...
void HCTracking::LoadConfig(const TDatime &date)
{
    ...
    const char *fPrefix = "bb.gem.";
    // Int_t err = Podd::LoadDatabase(file, date, request, fPrefix, 0, "HCTracking::LoadDB");
    Int_t err = Podd::LoadDatabase(file, date, request, fPrefix, 0, "HCTracking::LoadDB"); // The "1" after fPrefix means search up the tree
    fclose(file);
    if (err)
    {
        std::cout << __func__ << " XB Error: cannot load database" << std::endl;
    }
    ...
    if (fPedestalMode)
    { // then we will just dump raw data to the tree and/or histograms:
        // fZeroSuppress = false;
    ...
    std::vector<std::string> modules = Podd::vsplit(modconfig);
    if (modules.empty())
    {
    ...
    if (fModulesInitialized)
    {
        ...
        if (fModules.size() != modules.size())
        { // If the size of the configuration changed, we know that we have to re-initialize everything!
            Podd::DeleteContainer(fModules);
            fModules.resize(modules.size());
            fModulesInitialized = false;
        }
        else
        { // size stayed the same, but we need to check if any of the names changed:
            ...
            for (auto it = modules.begin(); it != modules.end(); ++it)
            {
                if (fModules[modcounter])
                {
                    ...
                    if (modname != *it)
                    { // name of one or more modules changed, assume we need to re-init everything:
                        Podd::DeleteContainer(fModules);
                        fModules.resize(modules.size());
                        fModulesInitialized = false;
                        break;
                ...
                { // one or more modules not allocated, re-init everything:
                    Podd::DeleteContainer(fModules);
                    fModules.resize(modules.size());
                    fModulesInitialized = false;
                    break;
    ...
    for (const auto &module : modules)
    {
        ...
        if (!fModulesInitialized)
        {
            ...
            // fModules[modcounter] = new GEMModule(module.c_str(), module.c_str(), this);
            fModules[modcounter] = new GEMModule(module.c_str(), module.c_str());
            fModules[modcounter]->LoadConfig(date);
...
void HCTracking::End(int run_number)
{
    for (auto &i : fModules)
        i->End(run_number);
