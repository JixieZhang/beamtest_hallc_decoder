...
public:
    ...
    float GetEpicsValueByName(const std::string &name) const;
    ...
    const std::unordered_map<std::string, uint32_t> &GetEpicsMap() const {return epics_map;}
    std::string GetCurrentTimeStamp() const {return current_timestamp;}
